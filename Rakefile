...
# Called by the chef-zero instance in main.tf before any node is
# bootstrapped for the multi-node suite.
...
task :destroy_all do
  run_command('rm -f Gemfile.lock && rm -f Berksfile.lock && rm -rf cookbooks/')
end
